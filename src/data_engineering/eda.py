...
def under_resourced_regions(by_region: dict) -> list[dict]:
    ...
    biggest = ordered[-1]
    thin = [{"region": r, "tokens": t, "vs_median": t / median, "vs_max": t / biggest}
            for r, t in totals.items() if t < UNDER_RESOURCED_RATIO * median]
    return sorted(thin, key=lambda d: d["tokens"])
...
def build_report(chunks: list[dict], cleaning: dict, dedup: dict, chunking: dict,
    ...
    # Coverage gap: a regional corpus is only as usable as its thinnest region, so call
    # out any region carrying well under the typical regional volume.
    thin = under_resourced_regions(by_region)
    if thin:
        A("### ⚠ Under-resourced regions - priority for additional sourcing")
        A("")
        A(md_table(["region", "tokens", "vs. median region", "vs. largest region"],
                   [[r["region"], f"{r['tokens']:,}", f"{r['vs_median']*100:.0f}%",
                     f"{r['vs_max']*100:.0f}%"] for r in thin],
                   ["---", "---:", "---:", "---:"]))
        A("")
        names = ", ".join(f"**{r['region']}**" for r in thin)
        A(f"{names} fall{'s' if len(thin) == 1 else ''} below "
          f"{int(UNDER_RESOURCED_RATIO * 100)}% of the median regional token count. "
          f"Additional sourcing here would do more for regional balance than more volume "
          f"anywhere else in the corpus.")
        A("")
    ...
    # ---------------- examples ----------------
    # Section 5 is the only part of this report that reproduces source text verbatim.
    # When the corpus licence is unresolved, --no-examples drops it so the statistical
    # report can still be shared while the excerpts stay local.
    if not include_examples:
        ...
        A("---")
        A("")
        A("Generated by `src/data_engineering/eda.py`.")
        return "\n".join(L) + "\n"
...
def main() -> None:
    ...
    ap.add_argument("--no-examples", action="store_true",
                    help="Omit section 5 (example chunks). That section is the only place "
                         ...
                         "corpus licence is unresolved and the report needs to be shared.")
    args = ap.parse_args()
    ...
    thin = under_resourced_regions(by_region_of(chunks))
    for t in thin:
        print(f"[eda] UNDER-RESOURCED REGION: {t['region']} - {t['tokens']:,} tokens, "
              f"{t['vs_median']*100:.0f}% of the median region, {t['vs_max']*100:.0f}% of the "
              f"largest. Priority for additional sourcing.")
    if args.no_examples:
        print("[eda] section 5 (example chunks) omitted - no verbatim source text in this build")
